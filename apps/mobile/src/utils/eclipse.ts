...
export function kindCodeForRecord(e: EclipseRecord): "T" | "A" | "H" | "P" {
  const rawKind = String(e.kind ?? "").toUpperCase();
...
export function nasaGifUrlForRecord(e: EclipseRecord) {
  ...
  const blockStartYear = Number.isFinite(yearNum)
    ? Math.floor((yearNum - 1) / 100) * 100 + 1
    : 2001;
...
export function eclipseCenterForRecord(
  e: EclipseRecord | null,
